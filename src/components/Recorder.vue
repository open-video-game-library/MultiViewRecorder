<template>
    <div class="wrapper">
        <div class="movie-wrapper">
            <CameraSelect @change-camera="changeCamera(0, $event)" />
            <CameraSelect @change-camera="changeCamera(1, $event)" />
            <canvas ref="canvas" :width="canvasWidth" :height="canvasHeight">Canvas not available.</canvas>
            <CameraSelect @change-camera="changeCamera(2, $event)" />
            <CameraSelect @change-camera="changeCamera(3, $event)" />
        </div>
        <div>
            <MicSelect @change-mic="changeMic" />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        changeCamera(videoIndex, value) {
            ...
            switch(videoIndex) {
                case 0:
                    this.video[videoIndex].posX = 0
                    this.video[videoIndex].posY = 0
                    break
                case 1:
